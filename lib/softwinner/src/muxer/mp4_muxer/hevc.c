...
static int hvcc_write(ByteIOContext *pb, HEVCDecoderConfigurationRecord *hvcc)
{
    uint8_t i;
...
#endif
int ff_isom_write_hvcc(ByteIOContext *pb, const uint8_t *data,
...
static unsigned int get_hvcc_write_size(HEVCDecoderConfigurationRecord *hvcc)
{
    unsigned int hvccWriteSize = 0;
    uint8_t i;
    uint16_t j, vps_count = 0, sps_count = 0, pps_count = 0;
...
unsigned int FFIsomGetHvccSize(unsigned char *data, int size, int ps_array_completeness)
{
    unsigned int hvccSize = 0;
    int ret = 0;
    uint8_t *buf, *end, *start = NULL;
    HEVCDecoderConfigurationRecord hvcc;
    ...
    //ret = hvcc_write(pb, &hvcc);
    hvccSize = get_hvcc_write_size(&hvcc);
