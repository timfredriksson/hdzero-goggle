...
static ssize_t FsSimpleCacheWrite(FsWriter *thiz, const char *buf, size_t size)
{
    ...
    if(0 == size || NULL == buf)
    {
        LOGE("Invalid input paramter!");
        return -1;
    }
//	if(size > pCtx->mCacheSize)
//	{
//    	LOGD("(f:%s, l:%d)write[%d]KB[%d]bytes!", size/1024, size%1024);
//	}
    if(pCtx->mValidLen>=pCtx->mCacheSize)
    {
        LOGE("fatal error! [%d]>=[%d], check code!", pCtx->mValidLen, pCtx->mCacheSize);
        return -1;
    }
    if(pCtx->mValidLen>0)
    {
    ...
    if(nLeftSize>=pCtx->mCacheSize)
    {
        ...
        LOGV("[%d], direct fwrite[%d]kB!", size-nLeftSize, nLeftSize/1024);
        fileWriter(pCtx->mpStream, buf+(size-nLeftSize), nWriteSize);
        nLeftSize -= nWriteSize;
    }
    if(nLeftSize>0)
    {
        //LOGD("(f:%s, l:%d)need cache!");
...
FsWriter *initFsSimpleCache(struct cdx_stream_info *pStream, int nCacheSize)
{
	if(nCacheSize<=0)
	{
        LOGE("param[%d] wrong![%s]", nCacheSize, strerror(errno));
		return NULL;
	}
	...
	if (NULL == pContext) {
        LOGE("Failed to alloc FsSimpleCacheContext(%s)", strerror(errno));
		goto ERROR0;
	}
    ...
    if(ret!=0)
    {
        LOGE("fatal error! malloc [%d]kByte fail.", nCacheSize/1024);
        goto ERROR1;
    }
