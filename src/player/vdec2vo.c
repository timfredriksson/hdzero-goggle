...
static ERRORTYPE vdec2vo_createClockChn(Vdec2VoContext_t *vvCtx, CLOCK_CHN_ATTR_S* clockChnAttr)
{
    ...
    while (vvCtx->clkChn < CLOCK_MAX_CHN_NUM) {
        ...
        if (SUCCESS == ret) {
            bSuccessFlag = TRUE;
            LOGD("create clock channel[%d] success!", vvCtx->clkChn);
            break;
        } else if (ERR_CLOCK_EXIST == ret) {
            LOGD("clock channel[%d] is exist, find next!", vvCtx->clkChn);
            vvCtx->clkChn++;
        } else {
            LOGD("create clock channel[%d] ret[0x%x]!", vvCtx->clkChn, ret);
            break;
        }
    }
